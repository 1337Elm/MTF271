...
DNS_ww = np.genfromtxt("P10k.ww.txt",comments="%")
...
np.savetxt('largest-Re-10000-error-percent.txt', [largest_error_percent_b1,largest_error_percent_b2,largest_error_percent_b4])

np.savetxt('error-loss.txt', [test_loss,b1_std,b2_std,b4_std] )

# save model to disk
filename = 'model-Re-10000.pth'
torch.save(neural_net, filename)
dump(scaler_yplus,'scaler-yplus-Re-10000.bin')
dump(scaler_pk,'scaler-pk-Re-10000.bin')
...
np.savetxt('min-max-re-10000.txt', [b1_min, b1_max, b2_min, b2_max, b4_min, b4_max, pk_min, pk_max, yplus_min, yplus_max] )
...
plt.xlabel("$y^+$")
plt.ylabel("loss")
plt.savefig('loss-Re-10000.png')




# load k-omega data
komega=np.loadtxt('y_u_k_eps_uv_channel-10000-k-omega.txt')
y_komega=komega[:,0];
